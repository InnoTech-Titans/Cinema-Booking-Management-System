...
def manager_reservations_auxiliar():
    ...
    projections = model.Projection.query \
        .filter(
            ...
            model.Projection.day >= past,
            model.Projection.movie_id.is_not(None)  # Filter for non-null movie_id
        ) \
        .order_by(model.Projection.day.asc(), model.Projection.time.asc()) \
        .all()
    print(projections)
...
def compute_reserved_seats(id):
    ...
    sum_result = db.session.query(
        ...
        model.Reservation.projection_id == projection.id
    ).one()
